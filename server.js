...
function originalHome(html){
  ...
  <nav class="home-control" aria-label="Платформа менюси">
    <button type="button" onclick="show('track')"><span>🔎</span><b>Буюртмани текшириш</b></button>
    <button type="button" onclick="show('worker')"><span>👷</span><b>Ходим</b></button>
    <button type="button" onclick="show('admin')"><span>📊</span><b>Админ</b></button>
  </nav>`;
  ...
  const css=`<style id="original-uyga-design">
    ...
    .home-control{position:fixed;left:50%;transform:translateX(-50%);bottom:18px;top:auto;z-index:100000;display:flex;gap:8px;align-items:center;width:max-content;max-width:calc(100vw - 24px);padding:8px;border:1px solid rgba(255,255,255,.95);border-radius:18px;background:rgba(255,255,255,.97);box-shadow:0 12px 40px rgba(0,0,0,.25);backdrop-filter:blur(12px)}
    .home-control button{appearance:none;padding:11px 15px;border:0;border-radius:12px;background:#eef8f6;color:#087f79;font-weight:800;white-space:nowrap;box-shadow:inset 0 0 0 1px #dcebe8;cursor:pointer}
    .home-control button:hover{background:#087f79;color:#fff}
    .home-control span{margin-right:5px}
    @media(max-width:700px){
      ...
      .home-control{left:7px;right:7px;transform:none;bottom:8px;width:auto;max-width:none;justify-content:stretch;gap:5px;padding:5px;border-radius:15px}
      .home-control button{flex:1;padding:10px 5px;font-size:12px}
      .home-control button:first-child{flex:1.45}
    }
  </style>`;
